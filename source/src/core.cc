...
int CORE_LoadBmp(const char filename[], bool wrap)
{
  ...
  if (fd != -1)
  {
    ...
    if (hdr.mark[0] == 'B' && hdr.mark[1] == 'M')
    {
      ...
      glTexParameteri(
          GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST); // GL_LINEAR_MIPMAP_NEAREST
      glTexParameteri(
          GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST); // GL_LINEAR
      glTexParameteri(
          GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, wrap ? GL_REPEAT : GL_CLAMP);
      glTexParameteri(
          GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, wrap ? GL_REPEAT : GL_CLAMP);
...
void CORE_RenderCenteredSprite(vec2 pos, vec2 size, int texix, rgba color, bool additive)
{
  ...
  glTexCoord2d(g_textures[texix].w, 0.0);
  glVertex2f(p1.x, p0.y);
  glTexCoord2d(g_textures[texix].w, g_textures[texix].h);
  glVertex2f(p1.x, p1.y);
  glTexCoord2d(0.0, g_textures[texix].h);
  ...
  if (fd != -1)
  {
          ...
          && hdr.format[3] == 'E'))
    {
      ...
      for(;;)
      {
        ...
        if (chunkhdr.subChunkID[0] == 'f'
            && chunkhdr.subChunkID[1] == 'm'
            && chunkhdr.subChunkID[2] == 't'
            && chunkhdr.subChunkID[3] == ' ')
        {
          read(fd, &fmt, sizeof(fmt));
          lseek(fd, ((1 + chunkdatasize) & -2) - sizeof(fmt), SEEK_CUR); // Skip to next chunk
        }
        else if (chunkhdr.subChunkID[0] == 'd'
            && chunkhdr.subChunkID[1] == 'a'
            && chunkhdr.subChunkID[2] == 't'
            && chunkhdr.subChunkID[3] == 'a')
        {
          ...
          ALsizei al_frequency = READ_LE_WORD(fmt.sampleRate);
